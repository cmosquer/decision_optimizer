...
def main():
    ...
    output_scores_csv = f"/data/results/scores_and_gt.csv"

    # Configure scorers
    evaluated_score_columns = [
        'lung_normalized_score',
    ...
    ground_truth_df = pd.read_csv(ground_truth_csv).drop_duplicates(id_col).set_index(id_col)
    if subgroup_col is not None:
        ground_truth_df = ground_truth_df[ground_truth_df[subgroup_col] == subgroup_evaluated]
    if not os.path.exists(output_scores_csv) or debug:
        output_scores_df = ground_truth_df.copy()
    else:
        output_scores_df = pd.read_csv(output_scores_csv).set_index(id_col)
    if subgroup_col is not None:
        output_scores_df = output_scores_df[output_scores_df[subgroup_col] == subgroup_evaluated]
    parameters = {}
    ...
    if 'lung_normalized_score' in evaluated_score_columns:
        if 'lung_normalized_score' not in output_scores_df.columns:
            ...
            temp_df = pd.read_csv(csv_path).set_index(id_col)
    ...
    splitter = Splitter(split_level=None,  # 'patient',
    ...
    if binarize_mask_th is not None:
        parameters.update({'binarize_mask_th': binarize_mask_th})
    ...
    joined_df_train = output_scores_df[
        output_scores_df.index.isin(ground_truth_train.index)]  # SOPInstanceUID].reset_index(drop=False)
